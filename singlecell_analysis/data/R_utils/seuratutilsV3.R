...
NB.var.genes <- function(object, cells.use=NULL, genes.use = NULL, do.plot=TRUE,set.var.genes=TRUE,
  ...
  cells.use=set.ifnull(cells.use,colnames(GetAssayData(object = object)))
  genes.use=set.ifnull(genes.use, rownames(GetAssayData(object = object)))
  if (!reads.use){
    count.data=GetAssayData(object = object, slot = "counts")[genes.use,cells.use]  # modified by Orr
  } else{
  ...
  mv.df = data.frame(mean=mean_emp, variance=cv_emp, variance.expected=0, variance.standardized=0)  # modified by Orr
  rownames(mv.df)=names(mean_emp)
  object[["RNA"]]@meta.features = mv.df  # modified by Orr
  ...
  if (set.var.genes) { 
    object[["RNA"]]@var.features = pass.cutoff
    if (sort.results) {  # Orr added Seurat code
      object[["RNA"]]@meta.features <- object[["RNA"]]@meta.features [order(
        object[["RNA"]]@meta.features$variance,
        decreasing = TRUE
      ),]
    }
    return(object)
...
QC10x <- function(qc.files, qc.out.file, genic.out.file, gcdata = NULL, 
  ...
  if (!is.null(gcdata)) {  
    ...
    qc$`Estimated Number of Cells` <- sapply(gcdata, function(data) length(data@active.ident))
    qc$`Median Genes per Cell` <- sapply(gcdata, function(data) as.integer(median(data$nFeature_RNA)))
    qc$`Median UMI Counts per Cell` <- sapply(gcdata, function(data) as.integer(median(data$nCount_RNA)))
    qc$`Total Genes Detected` <- sapply(gcdata, function(data) nrow(GetAssayData(object = data)))
  }
...
MergeMultipleSeuratObjects <- function(gcdata, project) {
  ...
  gcdata.merge <- merge(gcdata[[names(gcdata)[1]]], y = gcdata[2:length(gcdata)],
                          add.cell.ids = sampleid, project = project)
  ...
  Idents(gcdata.merge) <- gcdata.merge$orig.ident
  return(gcdata.merge)
}
...
pt.size=1, same.scale=FALSE, reduction="umap", lower, upper) {
    ...
    #' @param reduction Choose dimensional reduction in plots: tsne or umap. Default is umap.
    ...
    if (same.scale == TRUE & feature.type == "gene") {
        ...
        lower=min(plot.df$val)#quantile(plot.df$val,.05)#
        upper=max(plot.df$val)#quantile(plot.df$val,.95)#
    }
...
MultipleFeaturePlotv2 <- function(gcdata, features="nGene", feature.type="meta", ncols=ceiling(sqrt(length(features))), pt.size=1, same.scale=FALSE) {
  #' This function takes a feature from the @meta.data of a Seurat object, or from the un-scaled gene expression 
  #' @data of a Seurat object, and plots it on a tSNE with a blue to yellow to red gradient of colors. 
  #' If more than one feature is supplied, the graphs are sub-plotted. Metadata features and genes cannot be plotted
  ...
  p <- ggplot(plot.df, aes(x=tSNE_1, y=tSNE_2)) + geom_point(aes(color=val), alpha=0.8, shape=16, size=pt.size) + 
    theme(aspect.ratio = 1) + scale_color_gradientn(colors=colors, limits=c(lower, upper))
  p <- p + theme(aspect.ratio=1, text = element_text(size=12), axis.text=element_text(size=12), 
                 strip.text.x = element_text(margin = ggplot2::margin(.1, 0, .1, 0, "cm")),
...
RankModuleScore <- function(gcdata, name.cluster, gs, plot.dir, ctrl=10, reduction="umap") {
    ...
    for (i in seq(gs$geneset.names)) {
        ...
        tryCatch({
            ...
            ggsave(paste0(plot.dir, reduction, "_", name, ".png"), width = 6, height = 6, dpi = 200,type="cairo")
        }, error=function(e) NA)
    }
...
RankModuleScorev2 <- function(gcdata, name.cluster, gs, plotdir, ctrl.size=10) {
  ...
  for (i in seq(gs$geneset.names)) {
    ...
    MultipleFeaturePlot(gcdata, features = name, feature.type = "meta", pt.size = .1)
    ggsave(paste0(plotdir, "tSNE_", name, ".png"), width = 4, height = 4, dpi = 200, type="cairo")
  }
...
single_plot = function(df, verbose=F, pt.size=1.5, light.theme=F, max.value=0, pt.stroke=0, cols=NULL, max.value.rel=0) {
	if(light.theme){
		plot_theme = theme_cowplot()
		if(is.null(cols)){cols=material.heat(25)}
	}else{
		if(is.null(cols)){cols=cubehelix2.16}
	...
	if(any(is.na(val.num)) | (all(val.num %% 1 == 0) & max(val.num) < 100)) ## if there's NAs, the value has strings. if they are all integers and in the 0-100 range, probably cluster labels
	{
		...
		color_theme = scale_colour_manual("", values=default.cols(n), guide=guide_legend(override.aes = list(size=3))) 
	}else{
	...
	rv = ggplot(df, aes(x=tSNE_1, y=tSNE_2, color=value)) + geom_point(size=pt.size, stroke=pt.stroke) + 
		color_theme + ggtitle(g) + formatting  + xlab("tSNE 1") + ylab("tSNE 2") 
	return(rv)
}

### Seurat-2 compatible plotting function. replaces feature.plot.scale
FeatureP <- function(obj, features, cols=NULL, pt.size=1.5, verbose=F, light.theme=F, max.value=0, pt.stroke=0, max.value.rel=0.9)
{	
   pList = list()
   for(i in 1:length(features))
   {
   		...
   		pList[[i]] <- single_plot(melt(FetchData(obj, c("tSNE_1", "tSNE_2", f)), id.vars=c("tSNE_1", "tSNE_2")), max.value.rel=max.value.rel,
   			max.value=max.value, verbose=verbose, light.theme=light.theme, pt.size=pt.size, pt.stroke=pt.stroke, cols=cols)
   }
   cowplot::plot_grid(plotlist = pList, ncol = NULL)
}
